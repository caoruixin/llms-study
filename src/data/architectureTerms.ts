...
export const TERMS: Record<TermId, ArchitectureTerm> = {
  ...
  mqa: term('mqa', 'MQA', 'Multi-query Attention', '多查询注意力', '多个 Query heads 共享一组 K/V；减少缓存与 Decode 读取量。', 'https://arxiv.org/abs/1911.02150'),
  gqa: term('gqa', 'GQA', 'Grouped-query Attention', '分组查询注意力', 'Query heads 分组共享 K/V，介于 MHA 与 MQA 之间。', 'https://arxiv.org/abs/2305.13245'),
  ...
  dsa: term('dsa', 'DSA', 'DeepSeek Sparse Attention', 'DeepSeek 稀疏注意力', 'Lightweight Indexer 筛选历史 token，再做主 Attention；索引器自身也需要计算和缓存。', hf + 'deepseek-ai/DeepSeek-V3.2-Exp'),
  ...
  msa: term('msa', 'MSA', 'MiniMax Sparse Attention', 'MiniMax 稀疏注意力', '通过 block-level retrieval 选择历史块，降低长上下文主 Attention 的读取与计算。', hf + 'MiniMaxAI/MiniMax-M3'),
  swa: term('swa', 'SWA', 'Sliding Window Attention', '滑动窗口注意力', '仅保留并读取最近窗口；超出窗口的细节需要其他全局层或记忆路径承接。', 'https://arxiv.org/abs/2310.06825'),
  csa: term('csa', 'CSA', 'Compressed Sparse Attention', '压缩稀疏注意力', '沿序列压缩历史，再做稀疏选择；与局部窗口配合，压缩缓存仍随上下文增长。', hf + 'deepseek-ai/DeepSeek-V4-Flash'),
  hca: term('hca', 'HCA', 'Heavily Compressed Attention', '高度压缩注意力', '用更高压缩率的历史摘要提供全局信息；并非固定大小的 recurrent state。', hf + 'deepseek-ai/DeepSeek-V4-Flash'),
