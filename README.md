...
| **Inference Pipeline** | `/inference` | Four panels: full serving stack (4 layers), prompt lifecycle simulation, **VRAM-wall calculator** (weights + KV cache with per-attention-type formulas), token economics |
...
All throughput/latency/VRAM numbers are **illustrative estimates from documented formulas, not benchmarks**, and the UI labels them as such. Model specs and prices carry their source URL and as-of date; verify against official sources before quoting them in the field.
