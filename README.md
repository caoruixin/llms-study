...
- **Benchmark provenance stays visible.** The inference KPI workbench keeps business targets, roofline estimates, and imported AIPerf measurements as distinct value types. AIPerf summary/sweep/server-metrics JSON or CSV is parsed entirely in the browser and is never uploaded or persisted. Goodput-based sizing also requires an explicit customer SLO, attainment gate, and run-comparability confirmation.
